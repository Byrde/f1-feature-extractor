...
describe("computeLongRunAveragePace", () => {
  it("returns average lap time and sample size for long-run stints excluding pit-out laps", () => {
    const stints = [
      createStint({ lapStart: 1, lapEnd: 6 }),
    ];
    const laps = [
  ...
  it("excludes outliers greater than 107% of stint mean", () => {
    const stints = [
      createStint({ lapStart: 1, lapEnd: 5 }),
    ];
    const laps = [
  ...
  it("returns null value and zero sample size when no long-run stints exist", () => {
    const stints = [
      createStint({ lapStart: 1, lapEnd: 3 }),
      createStint({ lapStart: 4, lapEnd: 4, stintNumber: 2 }),
    ];
    const laps = [
...
describe("computeDegradationRate", () => {
  it("computes positive slope for degrading lap times in long-run stint", () => {
    const stints = [
      createStint({ compound: "MEDIUM", lapStart: 1, lapEnd: 6 }),
    ];
    const laps = [
      createLap({ lapNumber: 1, isPitOutLap: true, lapDuration: 100 }),
      createLap({ lapNumber: 2, lapDuration: 90 }),
    ...
    expect(result[0].rSquared).toBe(1);
    expect(result[0].lapCount).toBe(5);
  });

  it("excludes outliers greater than 107% of stint mean", () => {
    const stints = [
      createStint({ compound: "HARD", lapStart: 1, lapEnd: 7 }),
    ];
    const laps = [
      createLap({ lapNumber: 1, isPitOutLap: true, lapDuration: 100 }),
      createLap({ lapNumber: 2, lapDuration: 90 }),
    ...
    expect(result[0].rSquared).toBeGreaterThan(0);
    expect(result[0].rSquared).toBeLessThanOrEqual(1);
  });

  it("returns empty array when no long-run stints exist", () => {
    const stints = [
      createStint({ lapStart: 1, lapEnd: 3 }),
      createStint({ lapStart: 4, lapEnd: 6, stintNumber: 2 }),
    ];
    const laps = [
      createLap({ lapNumber: 1, lapDuration: 88 }),
      createLap({ lapNumber: 2, lapDuration: 87 }),
...
describe("computeFuelCorrectedLongRunPace", () => {
  it("applies 0.06s/lap fuel correction to long-run lap times", () => {
    ...
    // Lap indices 0,1,2,3,4 -> corrections: 0, 0.06, 0.12, 0.18, 0.24
    // Corrected times: 90, 89.94, 89.88, 89.82, 89.76
    // Mean: 89.88
    expect(result.value).toBeCloseTo(89.88, 2);
    expect(result.sampleSize).toBe(5);
  });
  ...
  it("excludes pit-out laps and outliers before applying correction", () => {
    ...
    // Mean: 89.865
    expect(result.value).toBeCloseTo(89.865, 2);
    expect(result.sampleSize).toBe(4);
  });
...
describe("computeConsistency", () => {
  it("computes standard deviation and sample size of lap times in long-run stints", () => {
    const stints = [
      createStint({ lapStart: 1, lapEnd: 7 }),
    ];
    const laps = [
      createLap({ lapNumber: 1, isPitOutLap: true, lapDuration: 100 }),
      createLap({ lapNumber: 2, lapDuration: 90 }),
      ...
      createLap({ lapNumber: 5, lapDuration: 92 }),
      createLap({ lapNumber: 6, lapDuration: 90 }),
      createLap({ lapNumber: 7, lapDuration: 92 }),
    ];

    const result = computeConsistency(laps, stints);

    // Times: 90, 92, 90, 92, 90, 92 -> mean = 91
    // Squared diffs: 1, 1, 1, 1, 1, 1 -> variance = 1 -> std dev = 1
  ...
  it("excludes outliers greater than 107% of stint mean", () => {
    ...
    const laps = [
      ...
      createLap({ lapNumber: 7, lapDuration: 91 }),
    ];

    const result = computeConsistency(laps, stints);

    // After outlier exclusion: 90, 92, 90, 92, 91 -> mean = 91
    // Squared diffs: 1, 1, 1, 1, 0 -> variance = 0.8 -> std dev ≈ 0.894
    expect(result.value).toBeCloseTo(0.894, 2);
    expect(result.sampleSize).toBe(5);
  });
